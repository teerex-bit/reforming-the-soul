...
Content-free metadata only: `id uuid`, `user_id uuid`, `intent_id uuid not null`, `request_fingerprint text not null`, `mode ai_mode`, `stage stage_id`, `curriculum_version_id text`, `node_id text`, `status ai_outcome`, `model_id text not null`, `global_policy_version text not null`, `stage_policy_version text not null`, `mode_policy_version text not null`, `output_schema_version text not null`, timing/token-count fields nullable, timestamps. `pending` is the sole non-terminal outcome and exists only to reserve provider dispatch. Unique `(user_id,intent_id)`. No prompt, response, transcript, provider conversation ID, or journal-body column.

The server atomically reserves `(user_id,intent_id,request_fingerprint)` before dispatch while holding the same actor/curriculum advisory lock used by the Reflect progress mutation. Under that lock it resolves an existing owner intent first: fingerprint conflicts, terminal replay, pending state, and stale-lease terminalization remain deterministic even if curriculum has since advanced. Only creation of a new reservation requires the locked current-node eligibility recheck and owned-context materialization. Only the insert winner receives current owned context and may call the provider. A matching pending retry receives `in_progress`; a matching terminal retry receives content-free `already_completed`; a different fingerprint for the same owner/intent is rejected. A pending lease older than two minutes is atomically terminalized as `provider_error` and also returns `already_completed`; the same intent is never redispatched. The terminal response is deliberately not replayed because response persistence is prohibited in Phase 1.
