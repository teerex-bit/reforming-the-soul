...
Status: pre-SY2 foundation contract. This document proposes a future relationship; it does not authorize or implement a migration. The current app identifiers and historical source documents remain intact.
...
| See Clearly → See Yourself Clearly | SY1 — Facts and Interpretation; SY2 — Follow the Formation Chain; SY3 — The Learned Self-Story; SY4 — What Is Actually True About Me | SY1 implemented; SY2–SY4 planned |
...
SY1's permanent stored module ID is `see-clearly.sc1`. The proposed SY2 ID is `see-clearly.sc2`; neither label change nor participant navigation may rewrite existing SY1 identifiers or rows. The old twelve-module See Clearly master-map, six-plus-six lesson numbering in the clean curriculum, and public Overview page numbers remain content/history, not current Deep Dive module counts. The public Overview is reference material; guided Deep Dive is the participant application. See [the naming reconciliation](../curriculum/see-clearly-naming-reconciliation.md) for source-to-module mappings.

## Proposed SY1 → SY2 relationship (before SQL)

1. A future SY2 chain is an independently owned, participant-authored structured record attached to its own `see-clearly.sc2` module progress. Its source SY1 record ID is optional. A participant can instead trace a recent event that has no SY1 record; the chain must still save and remain usable.
