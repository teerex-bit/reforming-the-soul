...
create function rts_private.save_awaken_observation(
  p_event_text text,
...
begin
  ...
  if p_event_text is null or p_internal_response_text is null or p_body_cue_text is null
    or p_event_text = '' or p_internal_response_text = '' or p_body_cue_text = '' then
  ...
  if found and v_state.current_node_id = 'awaken.pay-attention.reflect'
    and v_state.completed_node_ids @> array['awaken.pay-attention.observe', 'awaken.pay-attention.inside', 'awaken.pay-attention.body'] then
    return query select v_state.current_node_id;
    return;
  end if;
  if found and v_state.current_node_id <> 'awaken.pay-attention.observe' then
    raise exception using errcode = '22023', message = 'Awaken observation is no longer the current curriculum interaction';
  end if;
  ...
  on conflict (user_id, curriculum_version_id) do update
  set current_node_id = excluded.current_node_id,
      ...
      completed_node_ids = (
        ...
        from (
          ...
          from unnest(public.user_curriculum_state.completed_node_ids || excluded.completed_node_ids) node_id
        ) complete
      ),
      updated_at = now();
