...
begin
  if new.link_type = 'awaken_to_see_clearly' then
    select j.curriculum_version_id,j.node_id into v_source_version,v_source_node
      from public.journal_entries j where j.id=new.source_journal_entry_id;
    select r.curriculum_version_id,r.node_id into v_target_version,v_target_node
      from public.formation_records r where r.id=new.target_formation_record_id;
    if v_source_version is null or v_target_version is null
