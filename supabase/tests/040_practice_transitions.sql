...
select throws_like(
  $$select * from public.transition_practice('82000000-0000-4000-8000-000000000001', 'draft', 0, 'open')$$,
...
select throws_like(
  $$select * from public.transition_practice('82000000-0000-4000-8000-000000000001', null, 0, 'open')$$,
...
select throws_like(
  $$select * from public.transition_practice('82000000-0000-4000-8000-000000000001', 'draft', null, 'open')$$,
  '%expected practice state, lock version, and target state are required%', 'null lock version cannot bypass optimistic locking');
select results_eq(
  $$select state::text, lock_version from public.transition_practice('82000000-0000-4000-8000-000000000001', 'draft', 0, 'open')$$,
...
select throws_like(
  $$select * from public.transition_practice('82000000-0000-4000-8000-000000000001', 'open', 1, 'reviewed')$$,
  '%invalid practice transition%', 'invalid state edges fail');
select results_eq(
  $$select state::text, lock_version from public.transition_practice('82000000-0000-4000-8000-000000000001', 'open', 1, 'waiting_for_real_life')$$,
  $$values ('waiting_for_real_life'::text, 2)$$, 'open transitions to waiting_for_real_life');
select throws_like(
  $$select * from public.transition_practice('82000000-0000-4000-8000-000000000001', 'waiting_for_real_life', 2, 'ready_to_review')$$,
  '%invalid practice transition%', 'generic transition cannot bypass required return creation');
select throws_like(
  $$select * from public.record_practice_return('82000000-0000-4000-8000-000000000001', null, 'outcome')$$,
  '%expected practice lock version is required%', 'return recording cannot bypass optimistic locking with null');
select results_eq(
  $$select state::text, lock_version from public.record_practice_return('82000000-0000-4000-8000-000000000001', 2, '  exact outcome text  ')$$,
...
select throws_like(
  $$select * from public.transition_practice('82000000-0000-4000-8000-000000000001', 'ready_to_review', 3, 'reviewed')$$,
  '%invalid practice transition%', 'generic transition cannot bypass required review journal creation');
select throws_like(
  $$select * from public.review_practice('82000000-0000-4000-8000-000000000001', null, 'review')$$,
  '%expected practice lock version is required%', 'review cannot bypass optimistic locking with null');
select results_eq(
  $$select state::text, lock_version from public.review_practice('82000000-0000-4000-8000-000000000001', 3, '  exact review text  ')$$,
...
select results_eq(
  $$select state::text, lock_version from public.transition_practice('82000000-0000-4000-8000-000000000001', 'reviewed', 4, 'closed')$$,
