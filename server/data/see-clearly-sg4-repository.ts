...
export type SG4Record = Readonly<{ situation: string; trustMeaning: string }>;
...
function databasePool() {
  ...
  if (!url) throw new Error('Database URL is required for SG4 persistence.');
...
export function seeClearlySG4Repository() {
  return {
    getRecord(actorId: string): Promise<SG4Record | null> {
      return authenticated(actorId, async client => {
        const row = (await client.query<{ situation: string; trust_meaning: string }>(
          `select situation,trust_meaning from public.see_clearly_sg4_records where user_id=$1 and module_id=$2`, [actorId, SG4_MODULE_ID],
        )).rows[0];
        return row ? { situation: row.situation, trustMeaning: row.trust_meaning } : null;
      });
    },
    saveSection(actorId: string, sectionId: string): Promise<void> {
      return authenticated(actorId, async client => {
        await client.query(`insert into public.deep_dive_module_progress(user_id,curriculum_version_id,module_id,last_section_id)
          ...
          where public.deep_dive_module_progress.completed_at is null`, [actorId, SG4_MODULE_ID, sectionId]);
      });
    },
    saveRecord(actorId: string, situation: string, trustMeaning: string): Promise<void> {
      if (!situation.trim() || !trustMeaning.trim()) throw new Error('A situation and trust statement are required to save.');
      return authenticated(actorId, async client => {
        await client.query(`insert into public.deep_dive_module_progress(user_id,curriculum_version_id,module_id,last_section_id)
          ...
          where public.deep_dive_module_progress.completed_at is null`, [actorId, SG4_MODULE_ID]);
        const progress = (await client.query<{ id: string }>(
          `select id from public.deep_dive_module_progress where user_id=$1 and module_id=$2 for update`, [actorId, SG4_MODULE_ID],
        )).rows[0];
        await client.query(`insert into public.see_clearly_sg4_records(user_id,progress_id,situation,trust_meaning)
          values($1,$2,$3,$4) on conflict(progress_id,user_id,module_id) do update set
          situation=excluded.situation,trust_meaning=excluded.trust_meaning,updated_at=now()`,
        [actorId, progress.id, situation, trustMeaning]);
      });
    },
    deleteRecord(actorId: string): Promise<void> {
      return authenticated(actorId, async client => {
        await client.query(`delete from public.see_clearly_sg4_records where user_id=$1 and module_id=$2`, [actorId, SG4_MODULE_ID]);
      });
    },
    saveReflection(actorId: string, body: string, advance: boolean): Promise<void> {
      return authenticated(actorId, async client => {
        const progress = (await client.query<{ id: string; completed_at: Date | null }>(
          `select id,completed_at from public.deep_dive_module_progress where user_id=$1 and module_id=$2 for update`, [actorId, SG4_MODULE_ID],
        )).rows[0];
        if (!progress) throw new Error('Open SG4 before reflecting.');
        ...
        [actorId, progress.id, SG4_REFLECTION_PROMPT_ID, body]);
