...
export type SG3Record = Readonly<{ observation: string }>;
...
function databasePool() {
  ...
  if (!url) throw new Error('Database URL is required for SG3 persistence.');
...
export function seeClearlySG3Repository() {
  return {
    getRecord(actorId: string): Promise<SG3Record | null> {
      return authenticated(actorId, async client => {
        const row = (await client.query<{ observation: string }>(
          `select observation from public.see_clearly_sg3_records where user_id=$1 and module_id=$2`, [actorId, SG3_MODULE_ID],
        )).rows[0];
        return row ? { observation: row.observation } : null;
      });
    },
    saveSection(actorId: string, sectionId: string): Promise<void> {
      return authenticated(actorId, async client => {
        await client.query(`insert into public.deep_dive_module_progress(user_id,curriculum_version_id,module_id,last_section_id)
          ...
          where public.deep_dive_module_progress.completed_at is null`, [actorId, SG3_MODULE_ID, sectionId]);
      });
    },
    saveRecord(actorId: string, observation: string): Promise<void> {
      if (!observation.trim()) throw new Error('An observation is required to save.');
      return authenticated(actorId, async client => {
        await client.query(`insert into public.deep_dive_module_progress(user_id,curriculum_version_id,module_id,last_section_id)
          ...
          where public.deep_dive_module_progress.completed_at is null`, [actorId, SG3_MODULE_ID]);
        const progress = (await client.query<{ id: string }>(
          `select id from public.deep_dive_module_progress where user_id=$1 and module_id=$2 for update`, [actorId, SG3_MODULE_ID],
        )).rows[0];
        await client.query(`insert into public.see_clearly_sg3_records(user_id,progress_id,observation)
          values($1,$2,$3) on conflict(progress_id,user_id,module_id) do update set
          observation=excluded.observation,updated_at=now()`,
        [actorId, progress.id, observation]);
      });
    },
    deleteRecord(actorId: string): Promise<void> {
      return authenticated(actorId, async client => {
        await client.query(`delete from public.see_clearly_sg3_records where user_id=$1 and module_id=$2`, [actorId, SG3_MODULE_ID]);
      });
    },
    saveReflection(actorId: string, body: string, advance: boolean): Promise<void> {
      return authenticated(actorId, async client => {
        const progress = (await client.query<{ id: string; completed_at: Date | null }>(
          `select id,completed_at from public.deep_dive_module_progress where user_id=$1 and module_id=$2 for update`, [actorId, SG3_MODULE_ID],
        )).rows[0];
        if (!progress) throw new Error('Open SG3 before reflecting.');
        ...
        [actorId, progress.id, SG3_REFLECTION_PROMPT_ID, body]);
