...
export type SG2Record = Readonly<{ situation: string; expectation: string }>;
...
function databasePool() {
  ...
  if (!url) throw new Error('Database URL is required for SG2 persistence.');
...
export function seeClearlySG2Repository() {
  return {
    getRecord(actorId: string): Promise<SG2Record | null> {
      return authenticated(actorId, async client => {
        const row = (await client.query<{ situation: string; expectation: string }>(
          `select situation,expectation from public.see_clearly_sg2_records where user_id=$1 and module_id=$2`, [actorId, SG2_MODULE_ID],
        )).rows[0];
        return row ? { situation: row.situation, expectation: row.expectation } : null;
      });
    },
    saveSection(actorId: string, sectionId: string): Promise<void> {
      return authenticated(actorId, async client => {
        await client.query(`insert into public.deep_dive_module_progress(user_id,curriculum_version_id,module_id,last_section_id)
          ...
          where public.deep_dive_module_progress.completed_at is null`, [actorId, SG2_MODULE_ID, sectionId]);
      });
    },
    saveRecord(actorId: string, situation: string, expectation: string): Promise<void> {
      if (!situation.trim() || !expectation.trim()) throw new Error('A situation and expectation are required to save.');
      return authenticated(actorId, async client => {
        await client.query(`insert into public.deep_dive_module_progress(user_id,curriculum_version_id,module_id,last_section_id)
          ...
          where public.deep_dive_module_progress.completed_at is null`, [actorId, SG2_MODULE_ID]);
        const progress = (await client.query<{ id: string }>(
          `select id from public.deep_dive_module_progress where user_id=$1 and module_id=$2 for update`, [actorId, SG2_MODULE_ID],
        )).rows[0];
        await client.query(`insert into public.see_clearly_sg2_records(user_id,progress_id,situation,expectation)
          values($1,$2,$3,$4) on conflict(progress_id,user_id,module_id) do update set
          situation=excluded.situation,expectation=excluded.expectation,updated_at=now()`,
        [actorId, progress.id, situation, expectation]);
      });
    },
    deleteRecord(actorId: string): Promise<void> {
      return authenticated(actorId, async client => {
        await client.query(`delete from public.see_clearly_sg2_records where user_id=$1 and module_id=$2`, [actorId, SG2_MODULE_ID]);
      });
    },
    saveReflection(actorId: string, body: string, advance: boolean): Promise<void> {
      return authenticated(actorId, async client => {
        const progress = (await client.query<{ id: string; completed_at: Date | null }>(
          `select id,completed_at from public.deep_dive_module_progress where user_id=$1 and module_id=$2 for update`, [actorId, SG2_MODULE_ID],
        )).rows[0];
        if (!progress) throw new Error('Open SG2 before reflecting.');
        ...
        [actorId, progress.id, SG2_REFLECTION_PROMPT_ID, body]);
