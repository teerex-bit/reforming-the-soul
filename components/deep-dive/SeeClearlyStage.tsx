...
export function SeeClearlyStage({ status, sy2Status, sy3Status, sy4Status, sg1Status, sg2Status }: { status: 'begin' | 'resume' | 'review'; sy2Status?: 'begin' | 'resume' | 'review'; sy3Status?: 'begin' | 'resume' | 'review'; sy4Status?: 'begin' | 'resume' | 'review'; sg1Status?: 'begin' | 'resume' | 'review'; sg2Status?: 'begin' | 'resume' | 'review' }) {
  ...
  return <section className="deep-dive-home deep-dive-home--see-clearly">
    ...
    <div className="see-clearly-movements">
      ...
      <section className="see-clearly-movement" aria-labelledby="see-god-heading">
        ...
        {sg2Status === 'review' ? <p className="see-clearly-movement__next">Up next: SG3 — Jesus Shows Us the Father</p> : null}
        ...
        <ol className="see-clearly-movement__lessons" aria-label="See God Clearly modules">
          {godLessons.map((title, index) => <li key={title} id={`see-god-sg${index + 1}`}><span className="see-clearly-movement__number">SG{index + 1}</span><span>{title}</span>
            ...
            {index === 1 && sg1Status === 'review' ? <Link className="button button--secondary" href={`/deep-dive/see-clearly/what-i-expect-from-god${sg2Status === 'review' ? '?section=entry' : ''}`}>{sg2Status === 'review' ? 'Review' : sg2Status === 'resume' ? 'Resume' : 'Begin'} SG2</Link> : null}</li>)}
        </ol>
      </section>
    </div>
  </section>;
