...
export function lessonState<S extends { id: string }>(input: {
  ...
  return {
    ...
    /** Never write progress while a completed lesson is being reviewed. */
    async advance(target: string, save: (id: S['id']) => Promise<unknown>, getCompletion: () => Promise<boolean>) {
    ...
    async finish(complete: () => Promise<unknown>, getCompletion: () => Promise<boolean>) {
